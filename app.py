...
# ✅ Updated: Smart Entry Zones, Chase Warning & Pullback Watch in Tab 1
...
with tab1:
    ...
    if not buy_signals.empty:
        # ✅ UPDATED: Execution-focused columns moved to front for visibility
        display_cols = [
            "Ticker", "Company", "Price(₦)", "Strength(%)",
            "Chase_Warning", "Pullback_Watch",  # ✅ Execution flags
            "Entry_Zone_Low", "Entry_Zone_High",  # ✅ Fair entry range
        ...
        st.caption("💡 **EXECUTION RULE:** Place LIMIT orders at or below `Entry_Zone_Low`. If `Chase_Warning=⚠️`, wait for pullback or skip. Never chase above `Entry_Zone_High`.")
    else:
    ...
    if not signals_df.empty:
        st.dataframe(signals_df[["Ticker", "Company", "Price(₦)", "Signal", "Strength(%)", "Chase_Warning", "Entry_Zone_Low", "Entry_Zone_High", "Liquidity_Flag", "Event_Tag", "Reasons"]], use_container_width=True, hide_index=True)
    else:
